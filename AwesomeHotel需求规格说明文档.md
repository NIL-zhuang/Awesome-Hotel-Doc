...
| 网站营销人员 | 网站营销人员巨有较好的计算机操作能力，会设计和更新网站营销策略，希望新系统能更加方便营销人员的工作。                                                                                                                                                                       |
...
**CON5**: 本项目将使用Spring Boot作为后端框架，使用Vue进行前端开发。
...
![front_end_pic](https://lemonzzy.oss-cn-hangzhou.aliyuncs.com/img/front_end.png)

#### 3.1.2 通信接口

**CI**: 系统和客户端使用RMI的方式进行通信。
...
一个经过系统验证的用户在选择进行酒店搜索后，输入相关的查询条件，系统根据用户的选择条件进行组合并筛选，将符合条件的酒店展示给用户
...
刺激： 用户可以限制搜索的酒店名称、房间（类型、原始价格区间、有空房期间（房间数量、入住日期，退房日期））、星级、评分区间

响应： 系统根据限制条件进行筛选，将符合条件的酒店展示给用户
...
##### 3.2.6.1 特征描述
  ...
  一个经过系统验证的酒店工作人员在确认用户入住时，更改该用户的订单状态为已执行。
...
|                   |                                                        |
| ----------------- | ------------------------------------------------------ |
| Order.Exec        | 系统应该允许酒店工作人员输入执行订单的命令             |
| Order.Exec.Status | 系统把被酒店工作人员执行的订单状态更改为已执行         |
| Order.Exec.End    | 完成状态更改后，系统提示执行成功，结束当前订单执行任务 |

#### 3.2.7 制定网站促销策略

##### 3.2.7.1 特征描述

  一个经过系统验证的网站营销人员开始制定网站促销策略（目前只考虑双11活动折扣），完成网站促销策略的表单填写，提交促销策略。
...
| WebsiteCoupon.Input.SelectType | 系统允许网站营销选择制定指定类型的网站促销策略（目前只支持双11活动折扣类型）                             |
...
**Safety2**: 系统应该按照用户身份验证用户的访问权限, 客户只能查看和维护自己的信息，酒店工作人员只能处理自己所属的酒店的工作，网站管理员不可修改用户的信用值。

**Safety3**: 系统中有一个默认的管理员账户，管理员账户信息只允许管理员用户修改账户密码，其余字段不可修改。

**Safety4**: 为防信息泄露，用户的账号、密码、姓名、联系方式必须密文存储。
...
**Usability2**: 本项目在桌面端和移动端的web浏览器中均可正常使用。

#### 3.3.4  可靠性

**Reliability1**: 多个用户请求发生冲突时，将根据冲突处理策略逐个进行处理。
